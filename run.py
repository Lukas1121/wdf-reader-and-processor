...
# file = "data/Blind_test/raw/Despiked/map3_Hery_despiked.wdf"
S_T = np.load("data/Blind_test/raw/numpy_arr/S_T_K_means.npy",allow_pickle=True)
for f in glob.glob("data/Blind_test/raw/Despiked/*"):
    file = f
    ...
    obj = open_wdf.CLS(file=file,c=c,pure_spectra_path=path_pure_spectra,remove_cosmic_rays=False)
    ...
    # S_T,_ = obj.unpack_components(path_pure_spectra,save_S_T=False)


    # plots = open_wdf.Plotting(file=file)
    # plots.quick_plot(wn,*S_T)
    ...
    def least_sq_loop(D,S_T,similarities_dict={"apifil":[],"butamben":[],"capryol":[],"transcutol":[]}):
        ...
        for i in range(len(D)):
            ...
            similarities_dict["capryol"].append(similarities[3])
            similarities_dict["transcutol"].append(similarities[2])
            sum_lst[0].append(similarities[0])
            sum_lst[1].append(similarities[1])
            sum_lst[2].append(similarities[3])
            sum_lst[3].append(similarities[2])
    ...
    A_capryol = open_wdf.CLS.refold_scores(obj,D_1d_i=np.array(similarities_dict["capryol"]),S_T=S_T,s_i=3)
    A_transcutol = open_wdf.CLS.refold_scores(obj,D_1d_i=np.array(similarities_dict["transcutol"]),S_T=S_T,s_i=2)
